...
async function login() {
  try {
    ...
    localStorage.setItem("token-type", response.data.data.token_type);

    userStore.fetchUser();
    router.push("/");
  } catch (error) {
    formAlert.value.alert = true;
...
<template>
  <form>
    ...
    <div v-if="formAlert.alert" class="text-red-500 text-sm mt-2 text-center">
